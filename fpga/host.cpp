...
void execute_dag(cl::Context& context, cl::Device& device) {
    ...
    cl::CommandQueue ooo_queue(context, device, CL_QUEUE_PROFILING_ENABLE | CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, &err);

    const int matrix_scale_factor = 2;
}

void get_first_device(cl::Context& context, cl::Platform& platform, cl::Device& device) {
...
void read_binary_file(const std::filesystem::path directory,
                      cl::Program::Binaries out) {

  for (auto entry : std::filesystem::directory_iterator{directory}) {
    if (entry.is_regular_file() && entry.path().extension() == ".xclbin") {
      ...
      info("Read {} bytes from {}", out.back().size(), entry.path().filename().c_str());
    }
  }
...
int main(int argc, char **argv) {
  ...
  read_binary_file(xlbin_path, bins);
